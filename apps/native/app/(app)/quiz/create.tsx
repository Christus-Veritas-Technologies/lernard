    ...
    <SafeAreaView className="flex-1 bg-background" edges={['top']}>
      <ScrollView className="flex-1" contentContainerClassName="px-4 pb-24 pt-6 gap-5">
        <CardContent className="mt-0 flex-row items-center justify-between rounded-2xl border border-slate-200 bg-white p-4">
          <Text className="text-2xl font-semibold text-slate-900">Create quiz</Text>
          <Button onPress={() => router.back()} title="Back" variant="secondary" />
        </CardContent>

        <Card className="p-4">
          <CardHeader className="gap-1">
            <CardTitle className="text-lg">Build a fresh quiz</CardTitle>
            <Text className="text-sm text-slate-500">
              Choose your source, set question count, and start practicing.
            </Text>
          </CardHeader>
          <CardContent>
            <QuizCreateForm />
